...
/**
 * @return CRM_Gdpr_Upgrader
 */
function _gdpr_civix_upgrader() {
  if (!file_exists(__DIR__ . '/CRM/Gdpr/Upgrader.php')) {
    return NULL;
  }
  else {
    return CRM_Gdpr_Upgrader_Base::instance();
  }
}
...
function _gdpr_civix_civicrm_managed(&$entities) {
  ...
  foreach ($mgdFiles as $file) {
    ...
    foreach ($es as $e) {
      if (empty($e['module'])) {
        $e['module'] = 'uk.co.vedaconsulting.gdpr';
      }
      $entities[] = $e;
...
function _gdpr_civix_civicrm_caseTypes(&$caseTypes) {
  ...
  foreach (_gdpr_civix_glob(__DIR__ . '/xml/case/*.xml') as $file) {
...
function _gdpr_civix_civicrm_angularModules(&$angularModules) {
  ...
  $files = _gdpr_civix_glob(__DIR__ . '/ang/*.ang.php');
  foreach ($files as $file) {
    $name = preg_replace(':\.ang\.php$:', '', basename($file));
    $module = include $file;
    if (empty($module['ext'])) {
      $module['ext'] = 'uk.co.vedaconsulting.gdpr';
    }
 ...
 * missing them.
 */
function _gdpr_civix_fixNavigationMenu(&$nodes) {
  $maxNavID = 1;
  ...
  _gdpr_civix_fixNavigationMenuItems($nodes, $maxNavID, NULL);
}

function _gdpr_civix_fixNavigationMenuItems(&$nodes, &$maxNavID, $parentID) {
  ...
  foreach ($origKeys as $origKey) {
    ...
    if (isset($nodes[$origKey]['child']) && is_array($nodes[$origKey]['child'])) {
      _gdpr_civix_fixNavigationMenuItems($nodes[$origKey]['child'], $maxNavID, $nodes[$origKey]['attributes']['navID']);
    }
  }
}
